...
<script>
import { getWorkOrderList, GetSales, GetOrderQuantity, GetLeadErboard } from '@/api/dashboard'
...
export default {
  ...
  methods: {
    async getWorkOrderList() {
      const { currentPageRecords } = await getWorkOrderList({
        start: this.value[0],
        end: this.value[1],
        pageSize: 10000000
      })
